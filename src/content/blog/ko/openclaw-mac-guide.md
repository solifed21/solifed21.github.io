...
> "Mac에서 터미널 명령어 실행할 때 `sudo`는 신중하게 사용해야 함. `sudo`는 관리자 권한으로 실행하는 건데, 잘못된 명령어에 sudo 붙이면 시스템 날릴 수 있음. 특히 인터넷에서 복사한 명령어에 무조건 sudo 붙이는 건 위험함. 뭘 하는 명령어인지 이해하고 실행하셈. 그리고 API 키는 절대 남에게 공유하지 말 것!"
